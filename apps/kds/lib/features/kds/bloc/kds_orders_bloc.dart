...
///  * on an SSE drop the stream is retried with exponential backoff up to 30 s;
...
class KdsOrdersBloc extends Bloc<KdsOrdersEvent, KdsOrdersState> {
  ...
  /// First reconnect delay — doubles every attempt up to [reconnectMaxDelay].
  ...
  /// newly arrived CONFIRMED orders for audio/visual feedback.
  ...
  Duration _backoffFor(int attempt) {
    final expoMs = reconnectBaseDelay.inMilliseconds << (attempt - 1);
    final cappedMs = expoMs > reconnectMaxDelay.inMilliseconds
        ? reconnectMaxDelay.inMilliseconds
        : expoMs;
    // Deterministic ±10 % jitter: spreads fleet reconnects without making
    // tests flaky.
    final jitterMs = (cappedMs * 0.1 * (attempt.isOdd ? 1 : -1)).round();
