...
abstract interface class KdsOrdersRepository {
  /// `GET /kitchen/orders/active` — full board snapshot (CONFIRMED, COOKING,
  ...
  /// (CONFIRMED → COOKING or COOKING → READY) with optimistic locking via
  /// [expectedVersion].
  ///
...
final class HttpKdsOrdersRepository implements KdsOrdersRepository {
  ...
  Future<KdsOrder> updateOrderStatus({
    ...
    try {
      ...
      throw KitchenOrderUpdateException(
        code ?? 'NETWORK',
        switch (code) {
