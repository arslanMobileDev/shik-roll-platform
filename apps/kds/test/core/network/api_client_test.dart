...
void main() {
  group('ApiClient auth interceptor (ADR-1618)', () {
    test('attaches the Bearer token from the provider', () async {
      final adapter = FakeHttpClientAdapter(statusCode: 200, body: {'ok': true});
      ...
      expect(
        adapter.requestHeaders.single['Authorization'],
        'Bearer jwt-abc',
      );
    });
    ...
    test('no token → no Authorization header', () async {
      ...
      expect(adapter.requestHeaders.single.containsKey('Authorization'), isFalse);
    });

    test('a 401 reports session invalidation exactly once (single-flight)', () async {
    ...
    test('401 from the PIN login endpoint is NOT a session invalidation', () async {
      final adapter = FakeHttpClientAdapter(statusCode: 401);
      var invalidations = 0;
      final client = ApiClient(
        baseUrl: 'http://test',
