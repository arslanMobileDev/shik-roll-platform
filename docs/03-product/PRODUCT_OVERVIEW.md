...
# Pre-Launch Catalog Preparation

Back Office, backend, PostgreSQL и Object Storage должны быть доступны до публикации Customer App в App Store и Google Play.

До мобильного релиза уполномоченный сотрудник может:

- создать категории и задать их порядок;
- импортировать или создать карточки блюд со стабильными идентификаторами;
- заполнить названия, описания, состав, вес, КБЖУ и цены;
- загрузить и проверить фотографии;
- отметить Popular, New и Featured позиции;
- настроить филиальную доступность и Stop List;
- сохранить каталог как Draft;
- проверить карточки в Preview;
- опубликовать готовый каталог.

Поток данных:

Back Office → Backend API → PostgreSQL / Object Storage → Customer App и другие клиентские интерфейсы.

Клиентские приложения всегда получают опубликованный каталог через Backend API. Изменение цены, описания, фотографии, порядка или доступности не требует обновления приложения.

Исходные JSON или CSV могут использоваться для первоначального импорта только после проверки владельцем. После импорта источником актуального каталога становится backend.
