...
| UJ-013 | Prepare and Publish Catalog | Menu Administrator | Must |
...
Подготовить полный каталог SHIK ROLL через Back Office до публикации клиентских приложений и опубликовать его без изменения приложения.
...
- доступен проверенный источник исходного меню или ручной ввод;
- Back Office, Backend API, PostgreSQL и Object Storage доступны.

## Main Flow

1. Открыть раздел Menu в Back Office.
2. Импортировать проверенный JSON или CSV либо создать категории вручную.
3. Проверить уникальные идентификаторы и порядок категорий.
4. Создать или обновить карточки блюд.
5. Заполнить название, описание, состав, вес, КБЖУ и цену.
6. Загрузить фотографии, назначить главную и установить порядок галереи.
7. Настроить Popular, New, Featured, Hidden и филиальную доступность.
8. Сохранить меню и карточки как Draft.
9. Проверить каталог в Preview.
10. Исправить ошибки валидации.
11. Опубликовать меню.
12. Проверить опубликованный каталог через тестовое клиентское приложение.

## Alternative Flows

- позиция без обязательных полей остаётся Draft;
- дублирующийся идентификатор отклоняется;
- неподдерживаемый файл изображения отклоняется;
- временно недоступное блюдо добавляется в Stop List для выбранного филиала;
- Hidden исключает блюдо из каталога без удаления исторических данных.

## Success Result

Опубликованный каталог доступен через Backend API и готов к отображению при первом запуске Customer App после публикации в App Store и Google Play.

## Acceptance Criteria

- каталог можно полностью подготовить до мобильного релиза;
- Draft не виден клиентам;
- Preview соответствует опубликованной карточке;
- публикация не требует новой сборки приложения;
- изменение цены, описания, фотографии или доступности отражается после получения данных с backend;
- все публикации и критические изменения фиксируются в Audit Log.
